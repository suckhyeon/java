...
public class Run {
	...
	public static void main(String[] args) {
		...
		Car c4 = new Sonata("흰색" , "하이브리드" , 2023);
		c4.drive();
		Sonata c5 = (Sonata)c4;
		 ...
		 * 
		 * 3. 다형성 : 부모타입으로부터 파생된 여러가지 타입의 자식 객체들을 부모클래스 타입 하나로 다룰 수 있는 기술
		 *	  다형성의 장점 : 
		 *
		 *    다형성 사용 전 : Sonata[] sonataArr = new Sonata[5];
	 *    				   sonataArr[0] = new Sonata("흰색", "가스", 2023);
		 *    			   Avante[] avanteArr = new Avante[5];
		 *    			   avanteArr[0] = new Avante("검정", "가솔", 2023);
		 *    
		 *    다형성 사용 후 : Car[] carArr = new Car[10];
		 *    			   carArr[0] = new Sonata("흰색", "가스", 2023);
		 *    			   carArr[1] = new Avante("검정", "가솔린", 2023);
		 *     			   carArr[2] = new Sonata("흰색", "가스", 2023);
		 *    			   carArr[3] = new Avante("검정", "가솔린", 2023);
		 *    			  
		 *    
		 ...
		 *    	} else if(carArr[i] istancof Avante){
